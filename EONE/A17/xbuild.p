...
type poly = array[0..3000,1..2] of longint;       (* coefficient basis     *)
                                                   (*  p[0,1] is the length *)
var
    i,j,k,n:longint;    
    ...
    AA:array[0..2200] of poly;

    used:array[1..3100000] of longint;
...
degtwo := 8; 
(*
 let us figure out what this kills  

( -1( aa,  a, (aa)a)+1( aa, aa,     a)a-2( aa,  a,     a)(aa)  )  

tang -(aa,a,(aa)a)+(aa,aa,a)a-2(aa,a,a)(aa)

   -((aa)a,a,a)+(aa,aa,a)+(aa,a,a)a

     ((aa)a,a,a) - (aa,aa,a) + (aa,a,aa) = (aa)(a,a,a) + (aa,a,a)a
                                 = 0           = 0
   -(aa,aa,a)+(aa,a,a)a
*)
...
AA[0][0,1] := 0;
   begin
         for i2 := start[degtwo] to stop[degtwo] do
           begin 
             D :=        pq(xx(a,a),        a, xx(xx(a,a),a) );
             E :=  xx(   pq(xx(a,a),  xx(a,a),            a  ),a);
             F :=  xx(   pq(xx(a,a),        a,            a  ), xx(a,a));
             
             G[1,2] := i2;

             Z := xx( xApyB(-1,D,1,xApyB(1,E,-2,F)),G);

             AA[0][0,1] := AA[0][0,1] + 1;
             AA[ AA[0][0,1] ] := Z;
             If(i2 < stop[degtwo]) then
                writeln(name,'"tang*',T[i2],'",')
             else
                writeln(name,'"tang*',T[i2],'"};')
           end;
   end;     
